...
class CircadianAnalysis(object):
    ...
    def __init__(self, input: {Wearable}):
        if type(input) is Wearable:
            self.wearables = [input]
            print("get wearable data.")
    ...
    def _get_SSA(self, w, col='hyp_act_x', freq='15T'): 
        # using hyp_act_a column to calculate circadian 
        ssa = defaultdict(dict)
        df = w.data[[w.get_experiment_day_col(), 'hyp_time_col', col]].resample('min', on='hyp_time_col').mean()[col]
        ...
        df_2 = df_1.between_time('06:00', '23:00')
        df_gk = df_2[0] + df_2[1]
        ...
        ssa['period'] = (ssa['acrophase'] - ssa['acrophase'].shift(1)).apply(lambda x: x / pd.Timedelta(minutes=1))
