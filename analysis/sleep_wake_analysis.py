...
class SleepWakeAnalysis(object):
    ...
    def __get_activity(wearable, activityIdx):
        # get activitycols[0] from wearable - only use [hyp_act_x]
        if wearable is None:
    ...
    def run_sleep_algorithm(self, algname, activityIdx=None, rescoring=False, on_sleep_interval=False, inplace=False, **kargs):
        ...
        for wearable in self.wearables:
            ...
            activity = wearable.data[activityIdx] # wearable.data['hyp_act_x']
    ...
    def webster_rescoring_rules(act, rescoring_rules="abcde"):
        ...
        rescoring_rules : TYPE, optional
            DESCRIPTION. The default is "abcde". Applies the Webster rescoring rules. 
            Add and delete letters (case insensitive) according to which rules you want to run.
