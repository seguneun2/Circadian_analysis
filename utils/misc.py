...
print("Imported")
def get_consecutive_series(df_in: pd.DataFrame, col: str):
    """
    ...
    df["_lenght"] = 0
    df["_seq_id"] = df.groupby((df[col].fillna(False) != df[col].shift(1)).cumsum()).ngroup()
...
def merge_sequences_given_tolerance(df_orig: pd.DataFrame, time_col: str, col_to_act: str, tolerance_in_minutes: int,
    ...
    if tolerance_in_minutes > 0:
        ...
        for next_sleep_seg_id in all_seq_ids[1:]:
            ...
            if start_time_next_segment - end_time_actual_seg <= datetime.timedelta(minutes=tolerance_in_minutes):
                ...
                df.loc[start_time_actual_seg:end_time_next_segment, col_to_act] = int(True)
            else:
                actual_sleep_seg_id = next_sleep_seg_id

    else:################################# int(True)
        df.loc[df_true_seq.index[0]:df_true_seq.index[-1], col_to_act] = int(True)
    ...
    return df[col_to_act].astype(bool), df[seq_length_col], df[seq_id_col]
